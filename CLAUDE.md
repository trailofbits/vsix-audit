...
- **Full API reference**: Use the `malwarebazaar-api` skill
...
Malware samples live in `trailofbits/vsix-zoo` (private) to avoid Dependabot alerts and AV triggers.
...
Pre-commit hooks (prek) run hygiene, security, quality, and test checks. Never use `--no-verify` to skip them.
...
2. Download by hash from VirusTotal or MalwareBazaar (use `malwarebazaar-api` skill)
