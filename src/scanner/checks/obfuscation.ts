...
function decodeSourceChar(content: string, index: number): SourceLetter {
  if (
    content[index] === "\\" &&
    content[index + 1] === "u" &&
...
function toUnicodeMatch(content: string, starts: number[], char: SourceLetter): UnicodeMatch {
  const { line, column } = findLineAndColumn(char.sourceStart, starts);
  return {
    line,
    column,
    matched: char.decoded,
    context: getContext(content, char.sourceStart, char.sourceLength),
  };
}

/**
 * Flags Cyrillic homoglyph attacks in source text.
 *
 * The scanner decodes `\uXXXX` and `\u{...}` source escapes before deciding
 * whether a character extends a token. Escaped whitespace and punctuation
 * still split tokens, so localization strings such as `Русский\u0020English`
 * do not become false mixed-script matches.
 *
 * Primary signal: a single contiguous letter run that mixes ASCII Latin with
 * Cyrillic look-alike chars (e.g., `gооgle`). Narrow secondary signal:
 * domain-like labels composed entirely of Cyrillic look-alikes (e.g.,
 * `https://раураӏ.com`).
 */
function detectMixedScriptCyrillicHomoglyphs(content: string): UnicodeMatch[] {
  ...
  for (const run of findLetterRuns(content)) {
    let hasLatin = false;
    let firstHomoglyph: SourceLetter | undefined;
    ...
    if (!firstHomoglyph) continue;

    if (hasLatin || (allCyrillicLookalikes && isDomainLabelContext(content, run))) {
      matches.push(toUnicodeMatch(content, starts, firstHomoglyph));
    }
  }
