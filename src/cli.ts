...
cli
  ...
  .action(async (target: string, options: CliScanOptions) => {
    try {
      ...
      const modules = options.module?.split(",").map((m: string) => m.trim());
      if (modules) {
        const invalid = modules.filter(
          (m) => !MODULE_NAMES.includes(m as (typeof MODULE_NAMES)[number]),
        );
        if (invalid.length > 0) {
          console.error(
            pc.red("Error:"),
            `Unknown module(s): ${invalid.join(", ")}. Valid modules: ${MODULE_NAMES.join(", ")}`,
          );
          process.exit(2);
        }
      }
...
function printTimings(timings: ModuleTimings): void {
  ...
  const entries: [string, number][] = [
    ["Load", timings.load],
    ...(timings.package !== undefined ? [["Package", timings.package] as [string, number]] : []),
    ...(timings.obfuscation !== undefined
      ? [["Obfuscation", timings.obfuscation] as [string, number]]
      : []),
    ...(timings.ast !== undefined ? [["AST", timings.ast] as [string, number]] : []),
    ...(timings.ioc !== undefined ? [["IOC", timings.ioc] as [string, number]] : []),
    ...(timings.yara !== undefined ? [["YARA", timings.yara] as [string, number]] : []),
  ];
